...
if (!db.data.chats[m.chat].modohorny && m.isGroup) throw '*[❗ИНФО❗] 𝙻𝙾𝚂 𝙲𝙾𝙼𝙰𝙽𝙳𝙾𝚂 +𝟷𝟾 𝙴𝚂𝚃𝙰𝙽 𝙳𝙴𝚂𝙰𝙲𝚃𝙸𝚅𝙰𝙳𝙾𝚂 𝙴𝙽 𝙴𝚂𝚃𝙴 𝙶𝚁𝚄𝙿𝙾, 𝚂𝙸 𝙴𝚂 𝙰𝙳𝙼𝙸𝙽 𝚈 𝙳𝙴𝚂𝙴𝙰 𝙰𝙲𝚃𝙸𝚅𝙰𝚁𝙻𝙾𝚂 𝚄𝚂𝙴 𝙴𝙻 𝙲𝙾𝙼𝙰𝙽𝙳𝙾 #включить порно*'
switch (command) {
case "pack": 
let url = await pack[Math.floor(Math.random() * pack.length)]
conn.sendButton(m.chat, `_🥵 уффф 🥵_`, author, url, [['🔄 ДАЛИЕ 🔄', `${usedPrefix + command}`]], m)
break
case "pack2":  
let url2 = await packgirl[Math.floor(Math.random() * packgirl.length)]
conn.sendButton(m.chat, `_🥵 уффф 🥵_`, author, url2, [['🔄 ДАЛИЕ 🔄', `${usedPrefix + command}`]], m) 
break
case "pack3":
let url3 = await packmen[Math.floor(Math.random() * packmen.length)]
conn.sendButton(m.chat, `_🥵 уффф 🥵_`, author, url3, [['🔄 ДАЛИЕ 🔄', `${usedPrefix + command}`]], m) 
break
case "videoxxx": case "vídeoxxx":    
let url4 = await videosxxxc[Math.floor(Math.random() * videosxxxc.length)] 
await conn.sendButton(m.chat, `*ᴅɪsғʀᴜᴛᴀ ᴅᴇʟ ᴠɪᴅᴇᴏ 🥵*`, author, url4, [['🔄 ДАЛИЕ 🔄', `${usedPrefix + command}`]], m)  
break  
...
await conn.sendButton(m.chat, `*ᴅɪsғʀᴜᴛᴀ ᴅᴇʟ ᴠɪᴅᴇᴏ 🥵*`, author, url5, [['🔄 ДАЛИЕ 🔄', `${usedPrefix + command}`]], m)    
break    
}}
